...
namespace QuanLib.Core
{
    ...
    public class Hex32
    {
        ...
        public Hex32(int value)
        {
            ...
            {
                hexString = new string('0', 8 - hexString.Length) + hexString;
            }
            for (int i = 0; i < 8; i++)
            {
        ...
        {
            if (length < 0)
                throw new ArgumentOutOfRangeException(nameof(length), length, "字符串长度不能小于0");

            if (Bit < length)
                return new string('0', length - Bit) + ToString();
            return ToString();
        }
